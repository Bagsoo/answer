...
void FirebaseAuthPlugin::SignInWithCredential(
  ...
  // Windows: avoid replying to Flutter from a non-platform thread.
  // Wait for completion and reply from the platform thread.
  while (signInFuture.status() == firebase::kFutureStatusPending) {
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
  }

  if (signInFuture.error() == 0) {
    // TODO: not the right return type from C++ SDK
    PigeonUserInfo credential = ParseUserInfo(signInFuture.result());
    ...
    result(FirebaseAuthPlugin::ParseError(signInFuture));
  }
}
