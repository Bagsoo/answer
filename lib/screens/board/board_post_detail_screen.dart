            ...
            if (reactions[currentUserId] != null) ...[
              ...
              TextButton(
                onPressed: () async {
                  Navigator.pop(ctx);
                  await _service.toggleReaction(
                      widget.groupId, widget.postId, reactions[currentUserId]!);
                },
                child: Text(l.remove,
          ...
          children: [
            ...
            if (isOwner)
              ListTile(
                ...
                onTap: () async {
                  Navigator.pop(ctx);
                  await _service.togglePinPost(
                      widget.groupId, widget.postId, isPinned);
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content: Text(isPinned ? l.postUnpinned : l.postPinned)));
                  }
                },
              ),
            // 수정 (내 글 또는 owner)
                ...
                onTap: () {
                  ...
                  showReportDialog(
                    ...
                    onSubmit: (reason, otherText) =>
                      ...
                      postId: widget.postId,
                      targetOwnerId: post['author_id'] as String? ?? '',
                      groupId: widget.groupId,
                      reason: reason,
                      otherText: otherText,
                    ),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              Navigator.pop(ctx);
              final ok =
                  await _service.deletePost(widget.groupId, widget.postId);
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(ok ? l.postDeleted : l.postSaveFailed)));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<Map<String, dynamic>?>(
        stream: _service.getPost(widget.groupId, widget.postId),
        builder: (context, postSnap) {
          ...
          for (final emoji in reactions.values) {
            reactionCounts[emoji as String] = (reactionCounts[emoji] ?? 0) + 1;
          }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      StreamBuilder<List<Map<String, dynamic>>>(
                        stream: _service.getComments(widget.groupId, widget.postId),
                        builder: (context, commentSnap) {
                          ...
                          return Column(
                            children: comments.map((comment) {
                              final isMyComment = comment['author_id'] == currentUserId;
                              final commentAt = comment['created_at'] as Timestamp?;
                              return Padding(
                                    ...
                                    if (isMyComment || isOwner)
                                      GestureDetector(
                                        onTap: () => _confirmDeleteComment(
                                            comment['id'] as String, l, colorScheme),
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              Navigator.pop(ctx);
              final ok = await _service.deleteComment(
